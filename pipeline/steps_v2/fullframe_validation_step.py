...
class FullFrameValidationStepImpl(FullFrameValidationStep):
    ...
    def process(
        ...
        validation_results = self.validator.validate_motion_tracks(video_path, motion_tracks, config)
    ...
    def _extract_model_contributions_from_sequences(
        self, validation_sequences: list[ValidationSequence]
            ...
            for detection in sequence.detections:
                ...
                if model_name not in contributions:
                    contributions[model_name] = ModelContribution(
                        model_name=model_name, total_detections=0, max_confidence=0.0, contributing_tracks=0
                    )

                contributions[model_name].total_detections += 1
                contributions[model_name].max_confidence = max(
                    contributions[model_name].max_confidence, detection.confidence
                )
