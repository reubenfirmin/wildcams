...
class AnimalClassificationStepImpl(AnimalClassificationStep):
    ...
    def __init__(self, config: ProcessingConfig):
        ...
        self.species_detected: set[str] = set()

        logger.info("🔬 Animal Classification Step initialized")
    ...
    def process(
        ...
        logger.info("🔬 Step 4: Animal Classification")
        ...
        try:
            ...
            for i, sequence in enumerate(validation_result.validated_sequences):
                ...
                if classified_sequence.is_confirmed_animal:
                    ...
                    logger.info(
                        f"🚫 Sequence {i + 1} filtered as non-animal (conf={classified_sequence.classification.animal_confidence:.3f})"
                    )
        ...
        logger.info(
            f"✅ Animal Classification completed: {len(animal_sequences)} animals confirmed, {self.sequences_filtered_non_animal} filtered"
        )
        if species_counts:
            logger.info(
                f"🐾 Species detected: {', '.join(f'{species}({count})' for species, count in species_counts.items())}"
            )

        return result

    def _classify_sequence(
        self, sequence: ValidationSequence, video_reader: VideoReader, video_path: Path
    ) -> ClassifiedSequence:
        """
        ...
        is_confirmed_animal = (
            classification_result.is_animal
            and classification_result.animal_confidence >= self.config.animal_confidence_threshold
        )

        return ClassifiedSequence(
            sequence=sequence, classification=classification_result, is_confirmed_animal=is_confirmed_animal
        )

    def _get_best_detection(self, sequence: ValidationSequence) -> Detection | None:
        """
    ...
    def _extract_detection_crop(self, detection: Detection, video_reader: VideoReader) -> np.ndarray | None:
        """
    ...
    def _create_non_animal_sequence(self, sequence: ValidationSequence) -> ClassifiedSequence:
        ...
        return ClassifiedSequence(sequence=sequence, classification=failed_classification, is_confirmed_animal=False)

    def _create_passthrough_result(
        self, validation_result: FullFrameValidationResult, start_time: float
    ) -> AnimalClassificationResult:
        """
        ...
        for sequence in validation_result.validated_sequences:
            ...
            classified_sequence = ClassifiedSequence(
                sequence=sequence, classification=passthrough_classification, is_confirmed_animal=True
            )
            animal_sequences.append(classified_sequence)
