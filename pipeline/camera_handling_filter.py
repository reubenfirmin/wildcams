...
class CameraHandlingFilter:
    ...
    def __init__(self):
        ...
        logger.info(f"📹 Camera Handling Filter initialized")
    
    def filter_motion_tracks_for_camera_handling(self, video_path: Path, motion_tracks: List[Dict], config, initial_track_count: int = None) -> List[Dict]:
        ...
        logger.info(f"  📊 Penalties: consistency={consistency_penalty:.2f}x, filtering={filtering_penalty:.2f}x (initial_tracks={self._initial_track_count}→{num_tracks})")
