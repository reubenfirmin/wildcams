...
class FrameExtractor:
    ...
    def __init__(self, max_frames: int = 20, sampling_strategy: str = 'uniform', debug_dir: Optional[Path] = None):
    ...
    def extract_frames(self, video_reader: VideoReader) -> Tuple[List[np.ndarray], List[float]]:
        ...
        if not video_reader.is_opened():
            logger.error(f"❌ Video reader not opened")
            return [], []
        ...
        logger.info(f"🎬 Processing {video_reader.video_path.name} ({video_reader.duration:.1f}s, {video_reader.total_frames} frames)")
