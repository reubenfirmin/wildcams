...
class ProcessedVideoTracker:
    ...
    def get_unprocessed_videos(self, video_extensions: List[str] = None) -> List[Path]:
    ...
    def get_filtered_videos(self, video_filter: List[str], video_extensions: List[str] = None) -> List[Path]:
        ...
        for video_filter_item in video_filter:
            if video_filter_item.isdigit():
                ...
                if matching:
                    ...
                    logger.warning(f"⚠️ Video index {video_index} not found (no IMG_{video_index:04d}.MP4 and index out of range)")
            else:
