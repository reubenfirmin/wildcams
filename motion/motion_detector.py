...
class MotionDetector:
    ...
    def _init_motion_detector(self, config: ProcessingConfig):
        ...
        if self.bg_subtractor is not None:
            logger.debug("🧹 Releasing old background subtractor")
    ...
    def open_video_stream(self, video_path: Path) -> cv2.VideoCapture | None:
    ...
    def detect_motion_regions(self, frame: np.ndarray, config: ProcessingConfig) -> list[tuple[int, int, int, int]]:
        ...
        for contour in contours:
            ...
            _center_x, _center_y = x + w // 2, y + h // 2
    ...
    def detect_motion_regions_with_subtractor(
        self, frame: np.ndarray, bg_subtractor, config: ProcessingConfig
