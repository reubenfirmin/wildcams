...
class MotionTracker:
    ...
    def find_consistent_motion_sequences_and_tracks(
        self, video_path: Path, fps: float, total_frames: int, config: ProcessingConfig
    ) -> list[dict]:
        ...
        for frame_idx in range(total_frames):
            ...
            if motion_regions:
                ...
                if current_sequence is not None:
                    ...
                    logger.info(
                        f"Motion sequence found: frames {current_sequence['start_frame']}-{current_sequence['end_frame']} ({duration:.2f}s)"
                    )
        ...
        logger.info("📊 MOTION DETECTION SUMMARY:")
        ...
        if motion_region_sizes:
            ...
            logger.info(
                f"  📏 Region sizes - Avg: {avg_region_size:.0f}px, Min: {min_region_size:.0f}px, Max: {max_region_size:.0f}px"
            )
            logger.info(
                f"  📦 Large regions (>100k px): {large_regions}/{len(motion_region_sizes)} ({100 * large_regions / len(motion_region_sizes):.1f}%)"
            )
        else:
        ...
        if frame_motion_count == 0:
            logger.warning("⚠️ NO MOTION DETECTED - Check parameters:")
            ...
            logger.warning("⚠️ MOTION DETECTED BUT NO SEQUENCES - Duration too strict:")
        ...
        for i, sequence in enumerate(motion_sequences):
            ...
            motion_track = {
                ...
                "avg_regions_per_frame": sum(len(regions) for regions in sequence["motion_regions"])
                / len(sequence["motion_regions"])
                if sequence["motion_regions"]
                else 0,
            }
    ...
    def _infill_motion_tracks(self, motion_tracks: list[dict], fps: float, config: ProcessingConfig) -> list[dict]:
            ...
            while changed:
                ...
                for j, track_b in enumerate(sorted_tracks):
                    ...
                    if self._can_infill_tracks(merged_track, track_b, fps, config):
                        logger.info(
                            f"[INFILL] Merging track_{track_b['track_id']} into track_{merged_track['track_id']}"
                        )
        ...
        logger.info(
            f"[INFILL] Result: {len(infilled_tracks)} tracks after infilling ({len(motion_tracks) - len(infilled_tracks)} tracks merged)"
        )
        ...
        if removed_count > 0:
            logger.info(
                f"[INFILL] Removed {removed_count} tracks shorter than {min_required_duration:.2f}s (max of min_track_duration={config.min_track_duration:.2f}s, min_consecutive_detection={config.min_consecutive_detection_seconds:.2f}s)"
            )
    ...
    def _merge_tracks_with_infill(self, track_a: dict, track_b: dict, fps: float) -> dict:
        ...
        regions_b[0][0] if regions_b and regions_b[0] else track_b.get("representative_region")
    ...
    def _print_track_summary(self, motion_tracks: list[dict], fps: float) -> None:
        ...
        for track in motion_tracks:
            ...
            if motion_regions:
                ...
                if end_region:
                    end_bbox_str = (
                        f"end_bbox:{end_region[0]:.0f},{end_region[1]:.0f},{end_region[2]:.0f},{end_region[3]:.0f}"
                    )
            ...
            logger.info(
                f"  🎯 track_{track_id}: frames:{start_frame}-{end_frame} ({len(frames)}frames) | "
        ...
        logger.info(
            f"📊 SUMMARY: {len(motion_tracks)} tracks | {total_frames} total frames | "
