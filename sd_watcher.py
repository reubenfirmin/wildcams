...
class SDCardWatcher:
    def __init__(self):
        ...
        self.video_extensions = set(
            ext.strip().lower() 
            for ext in os.getenv('VIDEO_EXTENSIONS', 'mp4,mov,avi,mkv,m4v').split(',')
        )
        ...
        logger.info(f"🎥 SD Card watcher started")
    ...
    def get_removable_devices(self) -> List[str]:
        ...
        devices = []
        try:
            result = subprocess.run(['lsblk', '-rno', 'NAME,TYPE,RM,SIZE'], 
    ...
    def get_mounted_removable_devices(self) -> List[tuple]:
        ...
        try:
            ...
            result = subprocess.run(['findmnt', '-rno', 'SOURCE,TARGET,FSTYPE'], 
                                  capture_output=True, text=True, check=True)
            ...
            for line in result.stdout.strip().split('\n'):
                ...
                if len(parts) >= 2:
                    ...
                    try:
                        with open(f'/sys/block/{base_device}/removable', 'r') as f:
                            if f.read().strip() == '1':
