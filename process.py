...
def setup_logging_early() -> None:
    ...
    test_logger.info("📋 Test log message - logger configuration verified")
...
def main() -> None:
    ...
    try:
        ...
        print("🎬 Starting Next Generation wildlife video processing...")
        print("📊 Mode: Motion detection + temporal consistency + full-frame validation")
        print(
            f"🕒 Temporal parameters: {args.min_track_duration}s duration, motion gap {args.motion_tracking_gap_seconds}s, min consecutive detection {args.min_consecutive_detection_seconds}s"
        )

        logger.info("🎯 Processing strategy: Next Generation Temporal Consistency")
        ...
        batch_processor.process_all_videos(video_filter=video_filter)
