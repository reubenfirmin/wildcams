...
class EnsembleCoordinator:
    ...
    def run_multiscale_analysis(self, frame: np.ndarray, config, timestamp_seconds: float = 0.0) -> list[Detection]:
        ...
        for scaled_frame, scale_factor, scale_name in multiscale_frames:
            ...
            scale_detections = self.run_ensemble_detection(
                frame=scaled_frame, config=config, timestamp_seconds=timestamp_seconds, full_frame=scaled_frame
            )
    ...
    def check_extended_class_correlations(self, detections: list[Detection], timestamp: float) -> None:
        ...
        yolo_detections = [
            d
            for d in detections
            if any(d.source.startswith(model) for model in self.yolo_engine.get_supported_models())
        ]
