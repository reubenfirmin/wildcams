...
class MegaDetectorInferenceEngine:
    ...
    def run_detection(
        ...
        results = md_model.single_image_detection(rgb_frame, det_conf_thres=threshold)
    ...
    def get_supported_models(self) -> list[str]:
        ...
        megadetector_variants = ["MDV6-yolov9-e", "MDV6-yolov9-c", "MDV6-yolov10-e", "MDV6-yolov10-c", "MDV6-rtdetr-c"]
    ...
    def check_extended_class_correlations(
        self, detections: list[Detection], yolo_detections: list[Detection], timestamp: float
    ) -> None:
        """Check for correlations between MegaDetector extended classes and YOLO detections."""
        megadetector_extended = [
            d for d in detections if d.source.startswith("MDV6-") and getattr(d, "raw_class_id", None)
        ]
        ...
        for md_det in megadetector_extended:
            ...
            for yolo_det in yolo_detections:
                ...
                if iou > 0.3:  # Significant overlap
                    logger.warning("🚨🚨🚨 CORRELATION DETECTED 🚨🚨🚨")
                    logger.warning(
                        f"⚡ MegaDetector extended class {md_class} (conf={md_conf:.3f}) correlates with {yolo_source} (conf={yolo_conf:.3f})"
                    )
                    ...
                    logger.warning("🚨🚨🚨 END CORRELATION 🚨🚨🚨")
