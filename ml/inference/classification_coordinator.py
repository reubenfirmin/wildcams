...
class ClassificationCoordinator:
    ...
    def classify_ensemble(self, image_crop: np.ndarray) -> ClassificationResult:
        ...
        logger.info(
            f"🔬 Ensemble: animal={ensemble_result.is_animal}, species={ensemble_result.species} (conf={ensemble_result.animal_confidence:.3f}){approving_info}"
        )
