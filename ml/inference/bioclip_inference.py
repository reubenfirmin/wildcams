    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            self.model, _, self.preprocess = open_clip.create_model_and_transforms(
                model_name, pretrained=pretrained, cache_dir=str(self.cache_dir)
            )
            ...
            raise RuntimeError(
                f"❌ BioCLIP configured but dependencies missing: {e}. Install with: pip install open-clip-torch torch torchvision"
            ) from e
        ...
        try:
            ...
            with torch.no_grad():
                ...
                top_probs, top_indices = torch.topk(
                    probs[0], k=min(self.config.bioclip_top_k, len(self.wildlife_classes))
                )
