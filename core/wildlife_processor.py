...
class WildlifeVideoProcessor:
    ...
    def extract_features_from_detection(self, video_path: Path, detection: Detection, 
                                       video_reader: Optional[VideoReader] = None) -> Optional[List[float]]:
        """
        Extract ResNet features from a detection.
        
        Args:
            video_path: Path to video file
            detection: Detection dictionary with bbox and frame info
            video_reader: Optional pre-opened VideoReader for efficiency
            
        Returns:
            Feature vector if successful, None if failed
        """
        frame_idx = detection.frame_idx
        # ELIMINATED: to_list() method violates Phase 5.0 no-dictionary rule
        # Use direct bbox coordinates instead
        bbox = [detection.bbox.x1, detection.bbox.y1, detection.bbox.x2, detection.bbox.y2]
        
        # Read frame from video - use provided reader or create new one
        if video_reader is not None:
            success, frame = video_reader.get_frame_at_index(frame_idx)
        else:
        ...
        if not success or frame is None:
            logger.error(f"💥 Failed to read frame {frame_idx} from {video_path.name}")
            return None
        ...
        if features is not None:
            logger.info(f"🎯 Features extracted: conf={detection.confidence:.3f}, dims={len(features)}")
        else:
