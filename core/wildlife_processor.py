...
from core.functional_utils import collect_video_files, validate_video_filter
from ml.ensemble_wrapper import MLDetectionEnsemble
from pipeline.step_interface_v2 import PipelineOrchestrator
from pipeline.steps_v2 import CameraHandlingStepImpl, FullFrameValidationStepImpl, MotionDetectionStepImpl
from pipeline.steps_v2.animal_classification_step import AnimalClassificationStepImpl
from video_io import AnalysisWriter, ProcessedVideoTracker
...
class WildlifeVideoProcessor:
    ...
    def __init__(self, config: ProcessingConfig):
        ...
        if config.enable_animal_classification:
            ...
            logger.info("🔬 Step 4: Animal Classification enabled")
        else:
            logger.info("⏭️ Step 4: Animal Classification disabled")
        ...
        logger.info("🎯 Wildlife video processor initialized with modular pipeline")
        logger.info(f"📁 Video directory: {self.video_dir}")
        logger.info(f"🤖 ML ensemble: {len(config.ensemble_models)} models")

    def process_video(self, video_path: Path, force_reprocess: bool = False) -> VideoAnalysis | None:
        """
        ...
        validated_sequences: list = []
        if (
            hasattr(analysis, "fullframe_result")
            and analysis.fullframe_result
            and hasattr(analysis.fullframe_result, "data")
        ):
        ...
        if not analysis.validation_result.animals_detected:
            ...
            logger.info(
                f"✅ Pipeline completed: {analysis.validation_result.validated_sequences_count} validated sequences"
            )
            analysis._is_failure = False
            return analysis

    def get_video_files(self, video_filter: list[int | str] | None = None) -> list[Path]:
        """
    ...
    def get_unprocessed_videos(self, video_filter: list[int | str] | None = None) -> list[Path]:
        """
