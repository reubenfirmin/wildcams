...
class ConfigurationManager:
    ...
    def setup_common_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        # Video selection
        parser.add_argument(
            "--videos", "-v", nargs="+", help="Optional list of video indices (e.g. 7 8 9) or names to process"
        )
        ...
        parser.add_argument(
            "--video-dir", default=DEFAULT_VIDEO_DIR, help="Directory containing videos to process (default: ./videos)"
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--max-frames", type=int, default=20, help="Maximum frames to extract per video (default: 20)"
        )

        # Validation thresholds
        parser.add_argument(
            "--megadetector-high-conf",
            type=float,
            default=0.3,
            help="High confidence threshold for MegaDetector (default: 0.3)",
        )
        parser.add_argument(
            "--yolo-high-conf", type=float, default=0.4, help="High confidence threshold for YOLO models (default: 0.4)"
        )
        parser.add_argument(
            "--min-yolo-detections", type=int, default=3, help="Minimum YOLO detections for validation (default: 3)"
        )
        parser.add_argument(
    ...
    def setup_motion_detection_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--motion-method", choices=["MOG2", "KNN"], default="MOG2", help="Motion detection method (default: MOG2)"
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--min-motion-area", type=int, default=300, help="Minimum motion area threshold in pixels (default: 300)"
        )
        parser.add_argument(
            "--max-motion-area",
            type=int,
        ...
        parser.add_argument(
            "--motion-history", type=int, default=100, help="Motion detection background history frames (default: 100)"
        )
        parser.add_argument(
            "--max-regions-per-frame",
            type=int,
            default=10,
            help="Maximum motion regions to process per frame (default: 10)",
        )
        parser.add_argument(
            "--min-region-width", type=int, default=30, help="Minimum motion region width in pixels (default: 30)"
        )
        parser.add_argument(
            "--min-region-height", type=int, default=30, help="Minimum motion region height in pixels (default: 30)"
        )
        parser.add_argument(
            "--max-aspect-ratio",
            type=float,
        ...
        parser.add_argument(
            "--motion-margin", type=int, default=30, help="Margin to expand motion regions for ML context (default: 30)"
        )

        # Temporal consistency arguments (for Next-Gen processor)
        parser.add_argument(
            "--min-track-duration", type=float, default=0.1, help="Minimum track duration in seconds (default: 0.1)"
        )
        parser.add_argument(
            "--motion-tracking-gap-seconds",
            type=float,
            default=1.0,
            help="Maximum time gap for motion track linking in seconds (default: 1.0)",
        )
        parser.add_argument(
            "--min-consecutive-detection-seconds",
            type=float,
            default=0.2,
        ...
        parser.add_argument(
            "--min-track-frames", type=int, default=1, help="Minimum frames required for valid track (default: 1)"
        )
        parser.add_argument(
            "--track-search-seconds",
            type=float,
            default=2.0,
        ...
        parser.add_argument(
            "--enable-animal-classification",
            action="store_true",
            default=True,
            help="Enable Step 4 animal classification (default: True)",
        )
        parser.add_argument(
            "--skip-animal-classification",
            dest="enable_animal_classification",
            action="store_false",
        ...
        parser.add_argument(
            ...
            default="bioclip,deepfaune",
            help="Comma-separated list of classification models (default: bioclip,deepfaune)",
        )
        parser.add_argument(
            "--bioclip-top-k", type=int, default=5, help="Number of top species predictions from BioCLIP (default: 5)"
        )
        parser.add_argument(
            "--bioclip-threshold", type=float, default=0.30, help="BioCLIP animal detection threshold (default: 0.30)"
        )
        parser.add_argument(
            "--deepfaune-threshold",
            type=float,
    ...
    def parse_video_filter(self, args: Namespace) -> list[int | str] | None:
        """Parse video filter from command line arguments."""
        video_filter: list[int | str] | None = None
